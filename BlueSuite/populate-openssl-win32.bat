...
@rem OpenSSL 32-bit build.

@rem OSSL and BLUS will need editing to point to the correct directories.
@rem OSSL is the build output directory of the OpenSSL 32-bit build.
@rem BLUS is the BlueSuite source release directory to populate.
set "OSSL=C:\src\build-openssl-1.1.1i-win32"
set "BLUS=C:\sourceReleaseBlueSuite3xWindows-1.2.3.4"
@echo off

if not exist "%OSSL%" (
    echo %OSSL% does not exist. Update OSSL to be your OpenSSL 32-bit Windows build.
    goto :EOF
)
if not exist "%BLUS%" (
...
for %%f in (applink.c, opensslconf.h) do (
    xcopy "%OSSL%\include\openssl\%%f" "%INCL%\openssl\include\win\openssl\" /Y /I /F
)
for %%i in (Debug, Release) do (
    ...
    rem dll files
    for %%f in (libcrypto-1_1.dll, libssl-1_1.dll) do (
